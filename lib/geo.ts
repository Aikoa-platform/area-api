 ...
 * Combines multiple signals: exact match, prefix, contains, Levenshtein.
 */
export function fuzzyMatchScore(query: string, target: string): number {
  ...
  // Exact match ignoring case (but with diacritics)
  ...
  // Only accept if reasonably close (at least 60% similar)
  if (similarity >= 0.6) {
    return similarity * 0.5; // Scale down since it's not an exact/partial match
  }

  // Check if any word in target matches
  ...
  for (const word of targetWords) {
    ...
    const wordMaxLen = Math.max(normalizedQuery.length, word.length);
